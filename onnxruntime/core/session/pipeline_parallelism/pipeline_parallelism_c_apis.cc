...
ORT_API_STATUS_IMPL(CreatePipelineSession, _In_ const OrtEnv* env, _In_ const ORTCHAR_T* ensemble_config_file_path,
...
ORT_API_STATUS_IMPL(GetOutputValues, _In_ const OrtResponseBatch* resp_batch, _In_ size_t batch_idx, _In_ OrtAllocator* allocator,
  ...
  API_IMPL_BEGIN
  const auto* resp_vec = reinterpret_cast<const std::vector<onnxruntime::experimental::OrtResp>*>(resp_batch);
  if (resp_vec->empty()) {
    *output = nullptr;
    *output_count = 0U;
    return nullptr;
  ...
  IAllocatorDeleter ortvalues_alloc(reinterpret_cast<OrtValue**>(
                                        allocator->Alloc(allocator,
                                                         resp_vec[batch_idx].size() * sizeof(OrtValue*))),
  ...
  assert(created == resp_vec[batch_idx].size());
