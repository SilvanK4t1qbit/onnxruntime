...
class PipelineSession {
 ...
 private:
  ...
  OrtStatus* ProcessResponses(int num_reqs,
                              ...
                              ResponseQueue& resp_queue,
                              std::vector<OrtResp>& resp_list);
  void ParseEnsembleFile(const std::string& ensemble_config_file_path, PipelineConfig& ens);

  PipelineSession(const PipelineConfig& ens, const OrtEnv& env);
  void Init(PipelineConfig& ens0, const OrtEnv& env);
